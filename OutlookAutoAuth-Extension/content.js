...
async function doRun() {
    ...
    if (!email || !secret)
        return;
    // Step 1: email
    const emailField = document.querySelector("input[type='email'], input#i0116, input[name='loginfmt']");
    ...
    // Step 2: password
    const pwField = document.querySelector("input[type='password'], input#i0118");
    if (visible(pwField)) {
        if (password)
    ...
    const cantUseLink = Array.from(document.querySelectorAll("a")).find(el => /i can't use my microsoft authenticator app right now/i.test(el.innerText));
    if (cantUseLink) {
        await sleep(DELAY_MS);
        cantUseLink.click();
        return;
    ...
    if (/verify your identity|choose.*verification|verification method/i.test(bodyText)) {
        const codeOption = Array.from(document.querySelectorAll("a, div[role='button'], li, [data-value]")).find(el => /use a verification code|verification code|authenticator.*code|one.time/i.test(el.innerText));
        if (codeOption) {
            await sleep(DELAY_MS);
            codeOption.click();
    ...
    const otpField = document.querySelector("input#idTxtBx_SAOTCC_OTC, input[name='otc'], input[autocomplete='one-time-code']");
    if (visible(otpField)) {
        fill(otpField, await generateTOTP(secret));
        await sleep(DELAY_MS);
        click("#idSubmit_SAOTCC_Continue, #idSIButton9, input[type='submit']");
