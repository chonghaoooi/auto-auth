...
chrome.storage.local.get(['email', 'secret', 'password', 'githubSecret'], data => {
  if (data['email'])        $input('email').value        = data['email'];
  if (data['secret'])       $input('secret').value       = data['secret'];
  if (data['password'])     $input('password').value     = data['password'];
  if (data['githubSecret']) $input('githubSecret').value = data['githubSecret'];
